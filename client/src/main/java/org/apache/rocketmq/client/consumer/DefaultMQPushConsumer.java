...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
     ...
     * 如果消费群是最近创建的，以至于所订阅的最早消息尚未过期，这意味着消费群代表了最近启动的业务，则消费将从一开始就开始
     * </li>
     * <li>
     ...
     * 并发消息消费是处理队列最大跨度，默认2000，表示如果消息处理队列中偏移量最大的消息与偏移量最小的消息的跨度超过2000，则延迟50ms后再来取消息
     */
     ...
     * 消息并发消费是一次消费消息条数，通俗点数就是每次传入MessageListener#consumeMessage中的消息条数
     */
     ...
     * queue waiting.
     * 最大消息充实次数，如果消息消费次数超过maxReconsumeTimes，那么僵该消息转移到失败队列，等待被删除
     */
